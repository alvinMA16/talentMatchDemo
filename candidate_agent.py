...
class CandidateAgent:
    ...
    def respond(self, recruiter_message=None):
        """
        候选人agent响应招聘方或开始对话
        
        Args:
            recruiter_message: 招聘方的消息，如果为None表示开始对话
        ...
        log_processing_step("CANDIDATE_AGENT", "START", f"Processing message from recruiter")
...
"""
            ...
            if recruiter_message:
                messages.append({"role": "user", "content": recruiter_message})
                # 先将招聘方消息添加到chat历史中
            ...
            if not candidate_response:
                log_model_response("bedrock-claude-4-sonnet", "CANDIDATE_AGENT_RESPONSE", success=False, 
            ...
            # 记录原始响应以便调试
            log_processing_step("CANDIDATE_AGENT", "DEBUG", f"Raw model response: {candidate_response}")
            ...
            try:
                ...
                log_processing_step("CANDIDATE_AGENT", "DEBUG", f"JSON decode error: {str(json_error)}")
                ...
                try:
                    ...
                    if start_idx != -1:
                        ...
                        if brace_count == 0:
                            ...
                            log_processing_step("CANDIDATE_AGENT", "DEBUG", f"Extracted JSON part: {json_part}")
            ...
            log_model_response("bedrock-claude-4-sonnet", "CANDIDATE_AGENT_RESPONSE", success=False, error_msg=str(e))
            log_processing_step("CANDIDATE_AGENT", "ERROR", f"Error in candidate agent: {str(e)}")
            return {
                "type": "chatting",
                "reasoning": f"系统错误: {str(e)}，使用默认回复",
                "payload": f"抱歉，候选人代理遇到了技术问题：{str(e)}",
                "error": True
            }
